<template>
  <div id="results">
    <div class="row">
      <div class="col-2">
        <ResultCard class="h-100">
          <NumericResult v-bind:n="populationN" label="Injured patients presenting to ambulance service"/>
        </ResultCard>
      </div>
      <div class="col-4">
        <ResultCard class="h-100">
          <NumericResult v-bind:n="positiveN" label="Patients with major trauma"/>
        </ResultCard>
      </div>
      <div class="col-4">
        <ResultCard class="h-100">
          <NumericResult v-bind:n="negativeN" label="Patients without major trauma"/>
        </ResultCard>
      </div>
      <div class="col-2"></div>
    </div>
    ...
    <div class="mt-4 p-3 border rounded bg-white mb-3">
      <h4>Network Description</h4>
      <p>
        The trauma network has 1 Major Trauma Centre (MTC) and {{nTu}} Trauma Units (TU).
      </p>
      <p>
        There are approximately {{formatInt(populationN)}} 999 calls for non-trivial injured patients each {{ populationUnit }} in the trauma network.
        Of the {{formatInt(populationN)}} injured cases, approximately {{ getDisplayPercentage(eventRate)}} ({{ formatInt(positiveN) }} cases) would have major trauma.
        It is assumed {{ getDisplayPercentage(tuCatchment) }} of patients are injured within the catchment area of the MTC.
      </p>
      <p>
        ...
        assuming perfect compliance, this will result in {{ formatInt(toolPositives) }} MTC pre-alerts/trauma team activations per {{ populationUnit }}.
        Of these trauma calls, {{ formatInt(truePositives) }} would have major trauma, while {{ formatInt(falsePositives) }} would not have major trauma.
        The over-triage rate (1-PPV, or false detection rate) would therefore be {{ getDisplayPercentage(1 - ppv) }}.
        Of the {{ formatInt(positiveN) }} patients with major trauma,
        {{ formatInt(truePositives) }} cases would correctly have MTC pre-alerts/trauma team activations<span v-if="populationUnit == 'day'"> ({{ formatInt(truePositives * 7)}} per week)</span>,
        while {{ formatInt(falseNegatives) }} cases would incorrectly not receive MTC pre-alerts/ trauma team activations<span v-if="populationUnit == 'day'"> ({{ formatInt(falseNegatives * 7)}} per week)</span>.
...
import NumericResult from "./NumericResult";
...
export default {
  ...
  methods: {
    getDisplayPercentage: p => Math.round(p * 1000) / 10 + "%",
    formatInt: n => Math.round(n).toLocaleString()
  }
}
</script>
